...
Свод правил, обязательных при работе с кодовой базой.

## Комментарии в коде

- Пишите комментарии на русском языке.
- Для функций, методов, классов и типов используйте JSDoc-блок `/** ... */`, чтобы зафиксировать назначение, входы, выходы и возможные исключения.
- Первый абзац JSDoc должен давать краткое описание. Развёрнутые детали переносите в следующие абзацы после пустой строки, соблюдая деловой тон и фокус на сценариях применения, не следует раскрывать детали реализации.
...
- Описания полей оформляйте короткими однострочными `/** ... */` комментариями непосредственно перед объявлением; в остальных случаях используйте многострочные комментарии с соответствующими правилами.
...
1. `Token` не зависит от других частей и формирует фундамент модуля.
